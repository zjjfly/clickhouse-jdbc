...
public class ClickHouseResultSet extends AbstractResultSet {
   ...
   *
   * @return {@code true} if this result set has another row after the current cursor position, {@code false} else
  ...
  /**
   * Tries to parse the value as a timestamp using the connection time zone if applicable and return its representation
   * as milliseconds since epoch.
   *
   * @param colNum column number
   * @return timestamp value as milliseconds since epoch
   * @deprecated prefer to use regular JDBC API methods, e.g. {@link #getTimestamp(int)} or {@link #getObject(int,
   * Class)} using {@link Instant}
   */
  @Deprecated
  ...
  /**
   * Tries to parse the value as a timestamp and return its representation as milliseconds since epoch
   *
   * @param colNum   the column number
   * @param timeZone time zone to use when parsing date / date time values
  ...
  /**
   * Parse the value in current row at column index {@code colNum} as an array of long
   *
   * @param colNum column number
   * @return an array of longs
   * @throws SQLException if the value cannot be interpreted as {@code long[]}
  ...
  public String toString() {
    return "ClickHouseResultSet{" +
        "dateTimeTimeZone=" + (dateTimeTimeZone != null ? dateTimeTimeZone.toString() : "null") +
        ", dateTimeZone=" + (dateTimeZone != null ? dateTimeZone.toString() : "null") +
        ", bis=" + bis +
        ", db='" + db + '\'' +
        ", table='" + table + '\'' +
